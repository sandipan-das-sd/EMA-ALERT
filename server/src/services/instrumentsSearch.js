...
class InstrumentsSearchService {
  ...
  async initialize() {
    console.log('[InstrumentsSearch] Initializing instrument search service...');
  ...
  async _doUpdate() {
    ...
    try {
      ...
      const [nseData, bseData] = await Promise.all([
        this._downloadAndDecompress('https://assets.upstox.com/market-quote/instruments/exchange/NSE.json.gz'),
        this._downloadAndDecompress('https://assets.upstox.com/market-quote/instruments/exchange/BSE.json.gz')
      ]);
      ...
      console.log(`[InstrumentsSearch] Processing ${nseInstruments.length} NSE instruments`);
      ...
      console.log(`[InstrumentsSearch] Processing ${bseInstruments.length} BSE instruments`);
      bseInstruments.forEach(instrument => this._addInstrument(instrument));

      this.lastUpdated = new Date();
      const duration = Date.now() - startTime;
      console.log(`[InstrumentsSearch] Update completed in ${duration}ms. Total instruments: ${this.instruments.size}`);
  ...
  _addInstrument(instrument) {
    ...
    if (!instrument.instrument_key || !instrument.trading_symbol || !instrument.name) {
      return;
    }

    // Filter for specific segments we support
    const supportedSegments = [
      'NSE_EQ', 'NSE_FO', 'NSE_INDEX',
  ...
  search(query, options = {}) {
    ...
    const {
      segments = ['NSE_EQ', 'NSE_FO', 'NSE_INDEX', 'BSE_EQ', 'BSE_FO', 'BSE_INDEX'],
      limit = 50
    } = options;
    ...
    results.sort((a, b) => {
      const aExact = a.tradingSymbol.toLowerCase() === searchQuery ||
                    a.shortName.toLowerCase() === searchQuery;
      const bExact = b.tradingSymbol.toLowerCase() === searchQuery ||
                    b.shortName.toLowerCase() === searchQuery;
      
      if (aExact && !bExact) return -1;
      if (!aExact && bExact) return 1;
      
      const aStarts = a.tradingSymbol.toLowerCase().startsWith(searchQuery) ||
                     a.name.toLowerCase().startsWith(searchQuery);
      const bStarts = b.tradingSymbol.toLowerCase().startsWith(searchQuery) ||
  ...
  getStats() {
    ...
    for (const instrument of this.instruments.values()) {
      segmentCounts[instrument.segment] = (segmentCounts[instrument.segment] || 0) + 1;
    }
  ...
  startAutoUpdate() {
    const scheduleNext = () => {
      ...
      console.log(`[InstrumentsSearch] Next update scheduled for ${next6AM.toISOString()}`);
