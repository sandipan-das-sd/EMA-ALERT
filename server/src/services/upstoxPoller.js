  ...
  async function tick() {
    ...
    try {
      ...
      const normalizeKeys = (key) => {
        ...
        const seps = ["|", ":"];
        const vals = [val, val?.toUpperCase?.(), val?.toLowerCase?.()].filter(
          Boolean
        );
        const segs = [seg, seg?.toUpperCase?.(), seg?.toLowerCase?.()].filter(
          Boolean
        );
      ...
      const quotes = instrumentKeys.map((key) => {
        ...
        if (!q && (key.includes("NSE_FO") || key.includes("BSE_FO"))) {
          try {
              ...
              if (instrument && instrument.tradingSymbol) {
                ...
                const buildFOSymbolVariants = (inst) => {
                  ...
                  const isFut =
                    ...
                    /FUT/.test(
                      inst.instrumentType || inst.instrument_type || ""
                    );
                  const opt = (
                    inst.optionType ||
                    inst.option_type ||
                    (inst.tradingSymbol?.toUpperCase().includes("PE")
                      ? "PE"
                      : "CE")
                  ).toUpperCase();
                  const strikeRaw =
            ...
            if (process.env.DEBUG_UPSTOX) {
              console.warn(
                `[Poller] FO key resolution error for ${key}:`,
                e.message
              );
            }
          }
        ...
        return {
          ...
          ts: now,
          changePct:
            q.cp && q.cp > 0 ? ((q.last_price - q.cp) / q.cp) * 100 : null,
      ...
      const present = quotes.filter(
        (q) => !q.missing && typeof q.changePct === "number"
      );
